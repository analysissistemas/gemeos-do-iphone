  ...
  return (
    <>
      <CabecalhoPagina
        ...
        subtitulo="Arraste o negócio entre as etapas. Fechar e perder a venda abrem o fluxo de encerramento."
        acoes={
      ...
      <DndContext id="funil" sensors={sensores} collisionDetection={pointerWithin} onDragStart={(e) => setAtivo(itens.find((x) => x.id === Number(e.active.id)) ?? null)} onDragCancel={() => setAtivo(null)} onDragEnd={aoSoltar}>
        <div className="rolagem-fina -mx-4 flex snap-x snap-mandatory gap-3 overflow-x-auto px-4 pb-4 sm:-mx-6 sm:px-6">
          {ETAPAS.map((etapa) => (
            <Coluna key={etapa.id} etapa={etapa.id} rotulo={etapa.rotulo} cards={porEtapa[etapa.id]} aoAbrir={(id) => setDetalhe(id)} />
          ))}
        </div>
...
function Coluna({ etapa, rotulo, cards, aoAbrir }: { etapa: Etapa; rotulo: string; cards: CardNegocio[]; aoAbrir: (id: number) => void }) {
  ...
  return (
    <section
      ...
      aria-label={rotulo}
      className={cn(
        "flex w-[84vw] max-w-[320px] shrink-0 snap-start flex-col rounded-[20px] border bg-vidro transition sm:w-[290px] xl:w-auto xl:max-w-none xl:flex-1",
    ...
    >
      ...
      <div className="flex min-h-[120px] flex-1 flex-col gap-2 px-2.5 pb-3">
        {cards.length === 0 ? (
          ...
          cards.map((c) => <CartaoArrastavel key={c.id} c={c} aoAbrir={aoAbrir} />)
        )}
      </div>
    </section>
  );
...
function CartaoArrastavel({ c, aoAbrir }: { c: CardNegocio; aoAbrir: (id: number) => void }) {
  const { attributes, listeners, setNodeRef, isDragging } = useDraggable({ id: String(c.id) });
  return (
    <div ref={setNodeRef} {...listeners} {...attributes} className={cn("touch-manipulation outline-none", isDragging && "opacity-30")}>
...
function Cartao({ c, aoAbrir, flutuando }: { c: CardNegocio; aoAbrir?: () => void; flutuando?: boolean }) {
  ...
  return (
    <article
      ...
      className={cn(
        "cursor-grab rounded-2xl border border-linha bg-elevado p-3 text-left shadow-sm transition hover:border-linha-forte active:cursor-grabbing",
    ...
    >
      ...
      {c.valorProposta != null && c.valorAnunciado != null && <p className="num text-[11.5px] text-ink-3">Anunciado {brl(c.valorAnunciado)}</p>}
      ...
      <div className="mt-2.5 flex items-center gap-2 border-t border-linha pt-2 text-[11.5px] text-ink-3">
        <span className="grid size-5 place-items-center rounded-full bg-trilho text-[9.5px] font-semibold text-ink-2" title={c.responsavel ?? "Sem responsável"}>
